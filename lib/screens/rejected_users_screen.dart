...
class RejectedUsersScreen extends StatefulWidget {
  const RejectedUsersScreen({Key? key}) : super(key: key);

  @override
  _RejectedUsersScreenState createState() => _RejectedUsersScreenState();
}
...
class _RejectedUsersScreenState extends State<RejectedUsersScreen> {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text("Rejected Users",
            style: TextStyle(
                color: Colors.black, fontFamily: "Cabin", fontSize: 20)),
        centerTitle: true,
          ...
          child: Column(
            children: [
              ...
              ListView.builder(
                ...
                shrinkWrap: true,
                itemCount:
                    Get.find<ScreenController>().foundRejectedUsers.length,
                itemBuilder: (ctx, index) {
                  String userEmail = Get.find<ScreenController>()
                      .foundRejectedUsers[index]
                      .email!;
  ...
  void searchUser(String query) {
    if (query.isEmpty) {
      setState(() {
        Get.find<ScreenController>().foundRejectedUsers =
            Get.find<ScreenController>().rejectedUsers;
      });
    } else {
      final listUsers =
          Get.find<ScreenController>().rejectedUsers.where((users) {
      ...
      setState(() {
        Get.find<ScreenController>().foundRejectedUsers = listUsers;
      });
    }
  }
}
